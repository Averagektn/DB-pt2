...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            WHILE @@FETCH_STATUS = 0
            BEGIN
                INSERT INTO [m2m_books_genres] ([b_id], [g_id]) VALUES (@book_id, @genre_id);
                FETCH NEXT FROM [genre_cursor] INTO @genre_id;
            END;
            CLOSE [genre_cursor];
...
BEGIN
    ...
    SELECT @average_quantity = AVG([b_quantity]) FROM [books];
    IF @average_quantity > 50
    BEGIN
        ROLLBACK;
        RAISERROR ('Average quantity exceeds 50. Transaction rolled back.', 16, 1);
        RETURN;
    END
    COMMIT;
END
